window.initSwipers = function () {
  ...
  for (const key in carouselData) {
    ...
    if (swiperContainer) {
      ...
      // Ajuste dinâmico: Desativar loop, centeredSlides e ajustar slidesPerView se houver poucos slides
      const slidesCount = carouselData[key].length;
      if (slidesCount < 5) {
        baseConfig.loop = false; // Desativa o loop para evitar espaços em branco
        baseConfig.centeredSlides = false; // Desativa centralização para alinhar slides ao início
        // Ajusta centeredSlides nos breakpoints também
        baseConfig.breakpoints[1024].centeredSlides = false;
        baseConfig.breakpoints[768].centeredSlides = false;
        baseConfig.breakpoints[0].centeredSlides = false;
        // Ajusta slidesPerView nos breakpoints para não exceder o número de slides
      ...
      // Inicialização do Swiper (sem duplicação)
